...
class Payment extends CI_Model {
    ...
    {
        $select = "loans.loan_id,loan_payments.*, CONCAT(customer.first_name, ' ', customer.last_name) as customer_name, 
                   CONCAT(teller.first_name, ' ',teller.last_name) as teller_name";
    ...
    function save(&$payment_data, $payment_id = false)
    {
        if (!$payment_id or ! $this->exists($payment_id))
        {
            if ($this->db->insert('loan_payments', $payment_data))
            {
				...
				$data_update_loan = array();
		
				
		
				$payment_data['date_modified'] = time();
				$this->db->where('loan_id', $payment_data['loan_id']);
				$data_update_loan['loan_payment_date'] = date('Y-m-d',strtotime($payment_data['date_paid']));
				if($payment_data['paid_amount']!=0)
				{
					$data_update_loan['payment_done']      = 1;
				}
				
				$this->db->update('loans', $data_update_loan);
	 ...
	 function update(&$payment_data, $payment_id = false)
    {
			    $this->db->where('loan_payment_id', $payment_data['loan_payment_id']);
    ...
    {
        $this->db->select('loans.*,payment_schedules.name');
		$this->db->join('payment_schedules','payment_schedules.payment_schedule_id=loans.payment_scheds','left');
        $this->db->from('loans');
	...
	function get_loans_edit($customer_id)
    {
        $this->db->select('loans.*,loan_payments.amount_paid_rate,payment_schedules.name');
		$this->db->join('payment_schedules','payment_schedules.payment_schedule_id=loans.payment_scheds','left');
		$this->db->join('loan_payments','loan_payments.loan_id=loan_payments.loan_id','left');
        $this->db->from('loans');
        $this->db->where("loans.customer_id", $customer_id);
        $this->db->where("loans.customer_id >", 0);
        return $this->db->get()->result();
    }
