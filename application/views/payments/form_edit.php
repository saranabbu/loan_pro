...
<?php echo form_open('payments/update_payment/' . $payment_info->loan_payment_id, array('id' => 'payment_form', 'class' => 'form-horizontal')); ?>
    ...
    <div class="col-lg-12">
        <div class="inqbox float-e-margins">
            ...
            <div class="inqbox-content">
                        ...
                        <select id="loan_id" name="loan_id" class="form-control">
                            ...
                            <?php foreach ($loans as $loan): ?>
                                ...
                                <?php if ($loan['loan_id'] === $payment_info->loan_id) : ?>
                                    ...
                                    <?php $balance_amount = $loan['loan_amount']; 
										  $amount_paid_rate  = $loan['amount_paid_rate']?>                    
                                <?php endif; ?>
                                <option value="<?= $loan['loan_id'] ?>" <?= $selected; ?> data-balance="<?= $loan['loan_amount'] ?>"><?= "($ ".$loan['loan_amount'].") - ".$loan['interest_rate']." %" ?></option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <div class="input-group date">
                            ...
                            echo form_input(
                                    array(
                                        'name' => 'date_paid',
                                        'id' => 'date_paid',
                                        'value' => ($payment_info->date_paid !=""? date("d/m/Y",strtotime($payment_info->date_paid)) : date("d/m/Y")),
                                        'class' => 'form-control',
                                    )
                            );
                            ?>
                ...
                <div class="form-group"><label class="col-sm-2 control-label"><?php echo form_label($this->lang->line('payments_amount_rate') . ':', 'teller', array('class' => 'wide')); ?></label>
                    <div class="col-sm-10">
                        <?php
                        echo form_input(
                                array(
                                    'name' => 'paid_amount_rate',
                                    'id' => 'paid_amount_rate',
                                    'value' => $amount_paid_rate,
                                    'class' => 'form-control',
                                    'type' => 'number',
                                    'step' => 'any',
                                )
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function () {
					...
					interest= this.interest_rate ;
                    options.append($("<option />").val(this.loan_id).attr("data-balance", this.loan_balance).text(this.name + " ( $ " + this.loan_amount + ") - " + this.interest_rate +" %"));
                });
				
				
				$("#paid_amount_rate").val(amount*interest/100);
