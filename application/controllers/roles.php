...
class Roles extends Secure_area implements iData_controller {
    ...
    function view($id = -1)
    {
        ...
        $data['module_ids'] = json_decode($data["info"]->rights, TRUE);
        $data['module_ids'] = is_array($data['module_ids']) ? $data['module_ids'] : array();        
        $data['id'] = $id;
    ...
    function save($id = -1)
    {
        ...
        $data["rights"] = json_encode($this->input->post("rights"));
        ...
        foreach ($$controller_name->result() as $data)
        {
            ...
            foreach ($fields as $field)
            {
                ...
                else if ($field === 'rights')
                {
                    if ($data->$field !== "false")
                    {
                        $tmp[] = implode(", ",json_decode($data->$field, TRUE));
                    }
                    else
                    {
                        $tmp[] = "";
